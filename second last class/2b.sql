...
--
-- Host: 127.0.0.1
-- Generation Time: Dec 25, 2024 at 05:36 PM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `dataDelete` (IN `pn` VARCHAR(30))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `dataModf` (`pid` INT, `pn` VARCHAR(30), `pq` INT, `pp` INT, `pw` INT, `bid` INT)   BEGIN
    UPDATE products 
    SET name = pn, 
        quantity = pq,
        price = pp,
